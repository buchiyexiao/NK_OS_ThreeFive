...
/* [wikipedia]The simplest Page Replacement Algorithm(PRA) is a clock algorithm. The first-in, first-out
 * page replacement algorithm is a low-overhead algorithm that requires little book-keeping on
 * the part of the operating system. The idea is obvious from the name - the operating system
 * keeps track of all the pages in memory in a queue, with the most recent arrival at the back,
 * and the earliest arrival in front. When a page needs to be replaced, the page at the front
 * of the queue (the oldest page) is selected. While clock is cheap and intuitive, it performs
 * poorly in practical application. Thus, it is rarely used in its unmodified form. This
 * algorithm experiences Belady's anomaly.
 *
 * Details of clock PRA
 * (1) Prepare: In order to implement clock PRA, we should manage all swappable pages, so we can
...
/*
 * (3)_clock_map_swappable: According clock PRA, we should link the most recent arrival page at the back of pra_list_head qeueue
 */
static int
_clock_map_swappable(struct mm_struct *mm, uintptr_t addr, struct Page *page, int swap_in)
{
    ...
    list_add(head, entry);// 将新页插入到链表最后
...
_clock_swap_out_victim(struct mm_struct *mm, struct Page ** ptr_page, int in_tick)
{
   ...
   list_entry_t *p = head;
     while (1) {
         ...
         if ((*pte & PTE_D) == 1) {
             *pte &= ~PTE_D;// 如果dirty bit为1，改为0
         } 
           else {
             *ptr_page = ptr;
             list_del(p);
             break;// 如果dirty bit为0，则标记为换出页
         }
     }
     return 0;
}
