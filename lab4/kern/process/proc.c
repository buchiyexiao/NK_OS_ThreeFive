...
alloc_proc(void) {
     ...
     *       enum proc_state state;                      // Process state 进程所处状态，用于调度
     *       int pid;                                    // Process ID
     *       int runs;                                   // the running times of Proces
     *       uintptr_t kstack;                           // Process kernel stack  
//每个（kernel or client）线程都有一个内核栈，对于用户进程，该栈是在特权级改变使用于保存被打断的硬件信息的栈（2 pages in ucore）
     ...
     *       uintptr_t cr3;                              // CR3 register: the base addr of Page Directroy Table(PDT)  记录页表起始地址，本应和mm->pgdir 一样
     ...
     */
        ...
        proc->flags = 0;//标志位初始化
        memset(proc->name, 0, PROC_NAME_LEN + 1);//进程名初始化
    }
    return proc;
}
